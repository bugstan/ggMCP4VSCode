 ...
 * @param {string} pathInProject 要创建的文件路径
 * @param {string} text 要填充的文本内容
 */
async function testCreateNewFileWithText(port, pathInProject, text) {
    console.log(`\n========== 测试创建新文件并填充内容 ==========`);
    console.log(`要创建的文件路径: ${pathInProject}`);
    ...
    try {
        ...
        // 构造请求体
        const requestBody = buildRequestBody("create_new_file_with_text", { 
            pathInProject, 
            text 
        });
        
        // 获取标准API路径
        const apiPath = getStandardApiPath('create_new_file_with_text');
...
if (require.main === module) {
    (async () => {
        try {
            ...
            let filePath = '';
            let textContent = '这是测试内容，由create_new_file_with_text.js生成。\n时间戳: ' + new Date().toISOString();
            let textFile = '';
            ...
            if (!filePath) {
                ...
                console.log('用法: node test/create_new_file_with_text.js --path=/path/to/target/file [--text="新内容" | --file=/path/to/source/file]');
                process.exit(1);
            }
            ...
            await testCreateNewFileWithText(port, filePath, textContent);
        } catch (error) {
...
module.exports = { testCreateNewFileWithText };
