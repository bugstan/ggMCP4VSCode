...
export class InterceptorChain {
    ...
    public register(interceptor: Interceptor): void {
        ...
        this.sorted = false;
        log.info(
            `Registered interceptor: ${interceptor.name} with priority ${interceptor.priority}`
        );
    }
    ...
    private sortInterceptors(): void {
        if (!this.sorted) {
            ...
            const interceptorNames = this.interceptors
    ...
    public async processBeforeRequest(context: RequestContext): Promise<RequestContext | null> {
        ...
        for (const interceptor of this.interceptors) {
            try {
                ...
                if (result === null) {
                    log.info(
                        `Interceptor ${interceptor.name} terminated request processing for ${context.toolName}`
                    );
                    return null;
                }
     ...
     */
    public async processAfterResponse(
