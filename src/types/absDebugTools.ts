...
export abstract class AbsDebugTools<T = any> extends AbsTools<T> {
     ...
     */
    protected async createBreakpointLocation(
        filePath: string,
        line: number
    ): Promise<{
        uri: vscode.Uri;
    ...
    protected getBreakpointsAtLocation(uri: vscode.Uri, line: number): vscode.SourceBreakpoint[] {
        return this.getAllBreakpoints()
            ...
            .filter((bp) => {
                ...
                return location.uri.fsPath === uri.fsPath && location.range.start.line === line;
            });
    }
    ...
    protected addBreakpoint(
        ...
        const breakpoint = new vscode.SourceBreakpoint(new vscode.Location(uri, position), enabled);
    ...
    protected getRunConfiguration(configName: string): any | null {
        try {
            const launchConfigs =
                (vscode.workspace.getConfiguration('launch').get('configurations') as any[]) || [];
